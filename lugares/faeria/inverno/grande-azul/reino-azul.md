---
title: Reino Azul
description: 
published: true
date: 2026-01-29T16:49:21.139Z
tags: lugar, fortaleza
editor: markdown
dateCreated: 2026-01-29T15:42:29.911Z
---

# Reino Azul

> Não confundir com a região também chamada de Reino Azul: [Grande Azul](/lugares/faeria/inverno/grande-azul) {.is-info}

Reino Azul foi uma fortaleza existente no [grande azul](/lugares/faeria/inverno/grande-azul), no [inverno](/lugares/faeria/inverno) de [faéria](/lugares/faeria). A fortaleza habita uma família real, que governa a região.

O reino, e a fortaleza, foram estabelicidos após a família real deserdar uma guerra que julgaram sem sentido. Estabelecendo casas na região. Tomando conhecimento do senso de justiça da família real, outros passaram a buscar asilo e abrigo sob a família real. Com a morte da primeira geração de monarcas, um grande senso de luto e reminiscência ao passado tomou conta da população local, que moldou a região.

O luto, o sentimento de perda, e até mesmo o sentimento agridoce do que ficou no passado se tornaram traços culturais locais, com os festivais da lembrança e do choro. Mencionar o que os antepassados fariam, relembrar grandezas dos antigos reis e rainhas, fazem parte do vocabulário local como expressões e gírias.

A fortaleza foi a terra natal de [Amy](/individuos/personagens-de-jogadores/amy), aventureira da [Confraria da Rosa Negra](/faccoes/faccoes-independentes/confraria-da-rosa-negra) e princesa real. Sua mãe, [Nyx](/individuos/nyx), foi rainha do reino azul.

No ano 1705 da Era de Drafeon, no calendário imperial do [plano material](/lugares/plano-material), uma poderosa criatura conhecida apenas como [anjo azul](/individuos/lazuliel) atacou e subjugou o Reino Azul, sequestrando a família real. Com controle do reino, o anjo azul passou a ordenar buscas por itens aos súditos, tomando interesse peculiar sobre a princesa Amy. Em 1755 ED, o anjo azul enviou Amy ao plano material onde ela encontrou a Confraria da Rosa Negra.

## Arquitetura

A fortaleza em si possui três níveis: no primeiro habitam a população plebeia que serve a família real, além de mercadores fornecem aos plebeus. O segundo nível habita a nobresia, ambas intelectual e militar, com bibliotecas, laboratórios e forjas. O último nível é composto pelas habitações reais, com salões de festas, pomares e jardins gélidos.

Combinações de metais, rochas e gelo compõem a arquitetura local. Entalhes detalhados e esculturas pomposas enfeitam as quinas e fachadas da fortaleza. Contrastando o tamanho das fadas habitantes do palácio, a arquitetura local é grandiosa, com portas, janelas, salas e móveis muito maiores do que o tamanho necessário para seus habitantes, mas sempre com adaptações que permitam uso confortável. Salões e corredores altos e largos permitem voo livre e movimento ágil.

## Demografia

Majoritariamente, a fortaleza é habitada por [povo-fada](/fauna-e-flora/especies-inteligentes/povo-fada), mas outras espécies de fadas também habitam a fortaleza.

## Política

O Reino Azul é um governo monarca. A família real (não existe sobrenome), é líderada sempre por uma rainha, caso a rainha se case, seu marido se torna rei. Um rei sem rainha não pode existir, no entanto.

Para que uma nova rainha seja definida, uma princesa deve comer a [geleia real](/itens/geleia-real), criada de pólen mágico em um ritual. Ao comer a geleia, traços físicos da princesa se alteram drásticamente, com seu tamanho aumentando para o tamanho de um [humano](/fauna-e-flora/especies-inteligentes/humano) comum. Assim, a princesa se torna rainha, carregando ainda mais o peso emocional da região. Caso uma rainha esteja viva durante o processo, ela morre instantaneamente.

Fadas fêmeas da família real possuem sempre nomes com três letras.


# Informações
`Tipo` Fortaleza Reino
`População` 5.000
`Gentílico` Grande-Azuliano 
`Sub-locações` 
`Governante` [Anjo Azul](/individuos/lazuliel)/[Nyx](/individuos/nyx)
`Organizações Dominantes` [Corte Descontente](/faccoes/nacoes/corte-descontente)